/* ─────────────────────────────────────────────────────────────────────────────
 * bergamot-worker.cjs — Mozilla Bergamot çeviri worker'ı (VENDOR EDİLMİŞ KOPYA)
 *
 * Kaynak : @mkljczk/bergamot-translator 0.4.16 › worker/translator-worker.js
 *          (https://codeberg.org/mkljczk/bergamot-translator — MPL-2.0)
 *          Yukarı akış: mozilla/translations › inference
 * Lisans : Mozilla Public License 2.0 — bkz. LISANS.md. Bu dosya MPL-2.0
 *          kalır; değiştirilmiş hâli burada açıkça işaretlidir.
 *
 * NEDEN KOPYA: npm paketi olduğu gibi çalışmıyor (ölçüldü 20.09.2026):
 *   1. Paket "type": "module" ama worker require() kullanıyor → Node/ESM
 *      altında worker hiç açılmıyor. Uzantıyı .cjs yapmak bunu çözüyor.
 *   2. Worker dosya yolunu url.pathname ile açıyordu → Windows'ta 'C:\C:\...'
 *      ve Türkçe karakterlerde %XX kaçışı. Bu makinenin kullanıcı klasörü
 *      Türkçe karakter taşıyor; yamasız sürüm burada HİÇ çalışmıyordu.
 * Yamalar dosya içinde "İLGEZDİ YAMASI" diye işaretli, başka değişiklik yok.
 * ─────────────────────────────────────────────────────────────────────────── */
/**
    ...
    global.self = new class GlobalWorkerScope {
        ...
        async fetch(url, options) {
            if (url.protocol === 'file:') {
                ...
                // ⚠️ İLGEZDİ YAMASI: url.pathname Windows'ta '/C:/...' verir ve
                // Node onu 'C:\C:\...' diye çözer; ayrıca Türkçe karakterler %XX kalır.
...
class BergamotTranslatorWorker {
    ...
    loadModule() {
        return new Promise(async (resolve, reject) => {
            try {
                // ⚠️ İLGEZDİ YAMASI: wasm uygulamayla PAKETLENMEZ, kullanıcı indirir ve
                // userData altında durur; yolu initialize() seçeneklerinden gelir.
                const {pathToFileURL} = require(/* webpackIgnore: true */ 'node:url');
                const wasmUrl = this.options.wasmYolu
                    ? pathToFileURL(this.options.wasmYolu)
                    : new URL('./bergamot-translator-worker.wasm', self.location);
